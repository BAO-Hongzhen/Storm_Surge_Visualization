...
line_time_labels = []
for y in ys:
    label = ax.text(0, y, '', color='yellow', fontsize=8, ha='left', va='center', backgroundcolor='black', bbox=dict(facecolor='black', alpha=0.5, boxstyle='round,pad=0.1'))
    line_time_labels.append(label)
...
def animate(i):
    ...
    if i % data_speed == 0:
        ...
        amp = 10 + 80 * ((tide / tide_levels.max()) ** 1.5)
        width = 12
        left = max(0, pos-width//2)
        right = min(N_POINTS, pos+width//2)
        positions[line_idx, left:right] += amp * np.hanning(right-left)
        # 更新所有线左侧的时间文本，只有当前线显示时间，其余为空
        for idx_label, label in enumerate(line_time_labels):
            if idx_label == line_idx:
                label.set_text(str(dt))
            else:
                label.set_text("")
    ...
    return lines + [txt_bottom] + line_time_labels
